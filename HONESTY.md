...
| Synthetic Garmin fallback - last resort only; **30 days of real Garmin data is committed** (`backend/data/garmin_samples.json`) and used by default | `backend/app/sample_data/garmin_fallback.json` (fallback); `backend/data/garmin_samples.json` (real, committed) | Used only if the real export is ever removed | Always use live per-patient device data |
